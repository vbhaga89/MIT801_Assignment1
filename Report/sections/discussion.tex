...
grid search in terms of generalisation. Both have fairly similar parameters with the exception that the parameters obtained from Bayesian optimization are more specific which in turn 
results in a better performance. The main reason as to why this is the case boils down to essentially how each method works. Grid search can only linearly sweep through the grid to find the 
best combination whereas with Bayesian optimization, the model is able to logarithmically search through the space with its only constraints being the upper and lower limit of 
