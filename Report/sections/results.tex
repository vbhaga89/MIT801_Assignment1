...
Firstly, the learning rate was tuned in isolation. The default parameters of the \emph{SGDClassifier} model were used. The only parameters that were changed were the following:
...
Next, GridSearchCV was then tested to find the most optimal hyperparameters. The following grid space was used:
...
The GridSearchCV function was run with the mentioned grid space (80 combinations) on 5 folds. As a result a total of 400 fits were performed with results obtained for each possible configuration.
...
It was found that the best configuration (Rank 1 in Table \ref{gridsearch}) achieved a Test PR-AUC of 0.1068.

\subsection{HalvingGrid Search Hyperparameter Tuning}\label{halvinggridsearch_section}

Thereafter, HalvingGridSearchCV was then tested to find the most optimal hyperparameters. The same grid used in Section \ref{gridsearch_section} together with the same default parameters were used.
...
\subsection{Bayesian Optimization Hyperparameter Tuning}\label{bayesian_section}

Lastly, Bayesian optimization was then run using the default parameters with only the following three changes to ensure consistency:
...
In addition, the comparison between the Grid Search hyperparameter tuning method and the Bayesian Optimization hyperparameter tuning method can be seen in Table \ref{bayesian_grid}:

\begin{table}[htbp]
\caption{Bayesian optimization versus grid search comparison}
\centering
\resizebox{\columnwidth}{!}{
\begin{tabular}{|c|c|c|c|c|}
    \hline
    ...
    Bayesian Opt & 40 & 57.6 & 0.1073 $\pm$ 0.03073 & 0.1102\\
    \hline
\end{tabular}
}
\label{bayesian_grid}
...
Finally, the convergence plot showing the best objective score versus trial number can be seen in Figure \ref{convergence_plot}:
